...
describe ('ValidatorRules Unit Tests', () => {
   ...
   it('should throw a validation error when combine two or more validation rules',() => {
     ...
     expect(() => validator.required().string().maxLength(5)).toThrow(new ValidationError('The field must be less or equal than 5 characteres'));

     validator = ValidatorRules.values(null, 'field');
     expect(() => validator.required().boolean().maxLength(5)).toThrow(new ValidationError('The field is required'));
   
     validator = ValidatorRules.values(5, 'field');
     expect(() => validator.required().boolean().maxLength(5)).toThrow(new ValidationError('The field must be a boolean'));
